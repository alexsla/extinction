...
# set seed for birdroducibility
set.seed(42)
points_bg_bird <-
  bird_points[sample(1:nrow(bird_points), nrow(bird_points) / 10), ]
...
for (i in 1:length(bird_tax)) {
  ...
  if (nrow(points_occ) == 1) {
    failed <- c(failed, bird_tax[[i]])
    bird_sdm[i, 5] <- bird_tax[[i]]
    next
  }
  ...
  pdf(paste("output/SDM/Response Curves/", bird_tax[[i]], ".pdf", sep = ""),
      useDingbats = F)
  ...
  pdf(paste("output/SDM/Current/Maps/", bird_tax[[i]], "_prob.pdf", sep = ""),
      useDingbats = F)
  print(
    oz_map %>%
      ggplot() +
  ...
  bird_sdm[i, 1] <- as.character(eval@results[bestmod, ]$fc)
  bird_sdm[i, 2] <- as.numeric(eval@results[bestmod, ]$rm)
  bird_sdm[i, 3] <- eval@results[bestmod, ]$auc.train
  bird_sdm[i, 4] <- thr$spec_sens
  bird_sdm[i, 5] <- bird_tax[[i]]
  ...
  write_rds(current_predictions,
            paste("output/SDM/Current/Rasters/", bird_tax[[i]], sep = ""))
  write_sf(current,
  ...
  pdf(paste("output/SDM/Current/Maps/", bird_tax[[i]], "_pa.pdf", sep = ""),
      useDingbats = F)
  print(
    oz_map %>%
      ggplot() +
  ...
  write_rds(future_predictions,
            paste("output/SDM/Future/Rasters/", bird_tax[[i]], sep = ""))
  write_sf(future,
  ...
  pdf(
    paste("output/SDM/Future/Maps/", bird_tax[[i]], "_prob.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
  ...
  pdf(
    paste("output/SDM/Future/Maps/", bird_tax[[i]], "_pa.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
...
# export results file
write_csv(drop_na(bird_sdm), "output/SDM/bird_summary.csv")