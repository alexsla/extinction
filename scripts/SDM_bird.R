...
set.seed(42)
points_bg_bird <-
  bird_points[sample(1:nrow(bird_points), nrow(bird_points) / 1000), ]
...
for (i in 1:length(bird_tax)) {
  ...
  if (nrow(points_occ) == 1) {
    ...
    bird_sdm <- bind_rows(bird_sdm,
                         setNames(c(rep(NA, 16), bird_tax[[i]]), names(bird_sdm)))
    next
  }
  ...
  writeRaster(stack(lapply(1:length(future_predictions),
              ...
              paste("output/SDM/Future/Rasters/", bird_tax[[i]], ".tif", sep = ""))
  write_sf(future,
  ...
  pdf(
    paste("output/SDM/Future/Maps/", bird_tax[[i]], "_pa_meanDispersal.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
  ...
  pdf(
    paste("output/SDM/Future/Maps/", bird_tax[[i]], "_pa_minDispersal.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
  ...
  pdf(
    paste("output/SDM/Future/Maps/", bird_tax[[i]], "_pa_maxDispersal.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
