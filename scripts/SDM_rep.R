...
set.seed(42)
points_bg_rep <-
  rep_points[sample(1:nrow(rep_points), nrow(rep_points) / 10), ]
...
for (i in 1:length(rep_tax)) {
  ...
  if (nrow(points_occ) == 1) {
    failed <- c(failed, rep_tax[[i]])
    rep_sdm <- bind_rows(rep_sdm,
                         setNames(c(rep(NA, 16), rep_tax[[i]]), names(rep_sdm)))
    next
  }
  ...
  pdf(paste("output/SDM/Response Curves/", rep_tax[[i]], ".pdf", sep = ""),
      useDingbats = F)
  ...
  pdf(paste("output/SDM/Current/Maps/", rep_tax[[i]], "_pa.pdf", sep = ""),
      useDingbats = F)
  print(
    oz_map %>%
      ggplot() +
  ...
  writeRaster(stack(lapply(1:length(future_predictions),
              ...
              paste("output/SDM/Future/Rasters/", rep_tax[[i]], ".tif", sep = ""))
  write_sf(future,
  ...
  pdf(
    paste("output/SDM/Future/Maps/", rep_tax[[i]], "_prob.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
  ...
  pdf(
    paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa_meanDispersal.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
  ...
  pdf(
    paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa_minDispersal.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
  ...
  pdf(
    paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa_maxDispersal.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
...
# export results file
write_csv(drop_na(rep_sdm), "output/SDM/rep_summary.csv")