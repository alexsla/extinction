...
for (i in 1:length(rep_tax)) {
  ...
  rep_sdm[i, 1] <- as.character(eval@results[bestmod, ]$fc)
  rep_sdm[i, 2] <- as.numeric(eval@results[bestmod, ]$rm)
  rep_sdm[i, 3] <- eval@results[bestmod, ]$auc.train
  rep_sdm[i, 4] <- thr$spec_sens
  rep_sdm[i, 5] <- rep_tax[[i]]
  ...
  write_rds(current_predictions,
            paste("output/SDM/Current/Rasters/", rep_tax[[i]], sep = ""))
  write_sf(current,
  ...
  write_rds(future_predictions,
            paste("output/SDM/Future/Rasters/", rep_tax[[i]], sep = ""))
  write_sf(future,
  ...
  pdf(
    paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
