...
# import Australian reptile shapefiles
rep_dist <- readRDS("data/rep_dist.rds")
...
oz_map <-
  ne_countries(country = "Australia",
               scale = 10,
               returnclass = "sf") %>%
  st_transform(st_crs(rep_dist)) %>%
  st_simplify(preserveTopology = FALSE, dTolerance = 0.01)

# import point locality of Australian reptile occurrences
rep_points <- readRDS("data/rep_points.RDS") %>%
  st_as_sf(coords = c("decimalLongitude", "decimalLatitude"),
           crs = st_crs(oz_map))

# generate vector of reptile taxa
rep_tax <- unique(rep_points$Binomial)
rep_tax <- rep_tax[1:length(rep_tax)]
...
set.seed(42)
points_bg_rep <-
  rep_points %>% sample_frac(0.1)

points_bg_rep <-
  dismo::gridSample(st_coordinates(points_bg_rep), currentEnv, n = 1)

oz_grid <- st_make_grid(oz_map, cellsize = res(currentEnv) * 10)

# generate new output files if don't already exist
if(!file.exists("output/SDM/rep_summary.csv")){
  tibble(
    AUC = numeric(),
    BOYCE = numeric(),
    TSS = numeric(),
    ...
    write_csv("output/SDM/rep_summary.csv")
}

if(!file.exists("output/SDM/rep_failed.csv")){
  tibble(
    Binomial = character(),
    reason = character()
  ) %>%
    write_csv("output/SDM/rep_failed.csv")
}

# iterate loop over all reptile taxa
for (i in 1:length(rep_tax)) {
  if(rep_tax[[i]] %in% read_csv("output/SDM/rep_summary.csv")$Binomial)
    next
  ...
  points_occ <-
    st_coordinates(st_as_sf(
      rep_points %>% dplyr::filter(Binomial == rep_tax[[i]]),
      crs = st_crs(oz_map)
    )) %>%
    as_tibble()
  
  if (nrow(points_occ) < 10) {
    tibble(Binomial = rep_tax[[i]],
           reason = "not enough points") %>%
      write_csv("output/SDM/rep_failed.csv", append = T)
    tibble(AUC = NA,
           BOYCE = NA,
           TSS = NA,
           block_size = NA,
           ...
           n = nrow(points_occ),
           Binomial = rep_tax[[i]]) %>%
      write_csv("output/SDM/rep_summary.csv", append = T)
    next
  }
  ...
  points_bg <-
    points_bg_rep[lengths(st_intersects(st_as_sf(
      as.data.frame(points_bg_rep),
      coords = c("X", "Y"),
      crs = st_crs(oz_map)
    ), oz_bg)) > 0, ]
  ...
  if (is.null(cv_test_result)) {
    block_size <- rep_dist %>% filter(Binomial == rep_tax[[i]]) %>% st_area() %>% as.numeric() %>% sqrt() %>% round(0)
  } else {
    min_area <- rep_dist %>% filter(Binomial == rep_tax[[i]]) %>% st_area() %>% as.numeric() %>% sqrt() %>% round(0)
  ...
  ggsave(paste("output/SDM/Response Curves/", rep_tax[[i]], ".pdf", sep = ""),
  ...
  # projections
  mod.dir <- dir("occ.rep/models", full.names = T)
  ...
  ggsave(paste("output/SDM/Current/Maps/", rep_tax[[i]], "_prob.pdf", sep = ""),
         oz_map %>%
           ggplot() +
  ...
  writeRaster(current_predictions,
              paste("output/SDM/Current/Rasters/", rep_tax[[i]], ".tif", sep = ""),
              overwrite = T)
  write_sf(current,
           paste("output/SDM/Current/Shapefiles/", rep_tax[[i]], ".shp", sep = ""))
  
  # plot presence/absence based on threshold under current climate conditions
  ggsave(paste("output/SDM/Current/Maps/", rep_tax[[i]], "_pa.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
           ...
           geom_sf(
             data = st_as_sf(
               dplyr::filter(rep_points, Binomial == rep_tax[[i]]),
               crs = st_crs(oz_map)
             ),
             size = .1
           ) +
  ...
  writeRaster(do.call(c, lapply(1:length(future_predictions),
              ...
              paste("output/SDM/Future/Rasters/", rep_tax[[i]], ".tif", sep = ""), overwrite = T)
  write_sf(future,
           paste("output/SDM/Future/Shapefiles/", rep_tax[[i]], ".shp", sep = ""))
  ...
  ggsave(paste("output/SDM/Future/Maps/", rep_tax[[i]], "_prob.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
  ...
  ggsave(paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa_meanDispersal.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
  ...
  ggsave(paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa_minDispersal.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
  ...
  ggsave(paste("output/SDM/Future/Maps/", rep_tax[[i]], "_pa_maxDispersal.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
