...
for (i in 1:length(amph_tax)) {
  ...
  amph_sdm[i, 1] <- as.character(eval@results[bestmod, ]$fc)
  amph_sdm[i, 2] <- as.numeric(eval@results[bestmod, ]$rm)
  amph_sdm[i, 3] <- eval@results[bestmod, ]$auc.train
  amph_sdm[i, 4] <- thr$spec_sens
  amph_sdm[i, 5] <- amph_tax[[i]]
  ...
  write_rds(current_predictions,
            paste("output/SDM/Current/Rasters/", amph_tax[[i]], sep = ""))
  write_sf(current,
  ...
  write_rds(future_predictions,
            paste("output/SDM/Future/Rasters/", amph_tax[[i]], sep = ""))
  write_sf(future,
  ...
  pdf(
    paste("output/SDM/Future/Maps/", amph_tax[[i]], "_pa.pdf", sep = ""),
    useDingbats = F,
    width = 20,
    height = 14
  )
