...
# import Australian amphibian shapefiles
...
# import point locality of Australian amphibian occurrences
amph_points <- readRDS("data/amph_points.RDS") %>%
  st_as_sf(coords = c("decimalLongitude", "decimalLatitude"),
           crs = st_crs(oz_map))

# generate vector of amphibian taxa
...
if(!file.exists("output/SDM/amph_summary.csv")){
  tibble(
    AUC = numeric(),
    BOYCE = numeric(),
    TSS = numeric(),
    ...
    write_csv("output/SDM/amph_summary.csv")
}

if(!file.exists("output/SDM/amph_failed.csv")){
  tibble(
    Binomial = character(),
    reason = character()
  ) %>%
    write_csv("output/SDM/amph_failed.csv")
}

# iterate loop over all amphibian taxa
for (i in 1:length(amph_tax)) {
  if(amph_tax[[i]] %in% read_csv("output/SDM/amph_summary.csv")$Binomial)
    next
  ...
  if (nrow(points_occ) < 10) {
    tibble(Binomial = amph_tax[[i]],
           reason = "not enough points") %>%
      write_csv("output/SDM/amph_failed.csv", append = T)
    tibble(AUC = NA,
           BOYCE = NA,
           TSS = NA,
           block_size = NA,
           ...
           n = nrow(points_occ),
           Binomial = amph_tax[[i]]) %>%
      write_csv("output/SDM/amph_summary.csv", append = T)
    next
  }
  ...
  if (is.null(cv_test_result)) {
    block_size <- amph_dist %>% filter(Binomial == amph_tax[[i]]) %>% st_area() %>% as.numeric() %>% sqrt() %>% round(0)
  } else {
    min_area <- amph_dist %>% filter(Binomial == amph_tax[[i]]) %>% st_area() %>% as.numeric() %>% sqrt() %>% round(0)
  ...
  ggsave(paste("output/SDM/Response Curves/", amph_tax[[i]], ".pdf", sep = ""),
  ...
  # projections
  mod.dir <- dir("occ.amph/models", full.names = T)
  ...
  ggsave(paste("output/SDM/Current/Maps/", amph_tax[[i]], "_prob.pdf", sep = ""),
         oz_map %>%
           ggplot() +
  ...
  ggsave(paste("output/SDM/Current/Maps/", amph_tax[[i]], "_pa.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
  ...
  writeRaster(do.call(c, lapply(1:length(future_predictions),
              ...
              paste("output/SDM/Future/Rasters/", amph_tax[[i]], ".tif", sep = ""), overwrite = T)
  write_sf(future,
  ...
  ggsave(paste("output/SDM/Future/Maps/", amph_tax[[i]], "_prob.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
  ...
  ggsave(paste("output/SDM/Future/Maps/", amph_tax[[i]], "_pa_meanDispersal.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
  ...
  ggsave(paste("output/SDM/Future/Maps/", amph_tax[[i]], "_pa_minDispersal.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
  ...
  ggsave(paste("output/SDM/Future/Maps/", amph_tax[[i]], "_pa_maxDispersal.pdf", sep = ""),
         oz_map %>%
           ggplot() +
           geom_sf(aes(geometry = geometry), colour = NA) +
           geom_sf(
